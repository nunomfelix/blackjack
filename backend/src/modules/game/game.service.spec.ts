...
describe('GameController', () => {
  ...
  beforeEach(async () => {
    ...
    game.resetGame(100);
    jest.spyOn(game, 'resetGame').mockImplementation(function() {
      this.player.hand = new Hand();
      this.dealer.hand = new Hand();
      this.player.hand.addCard(new Card('hearts', '3'));
      this.player.hand.addCard(new Card('spades', '9'));
      this.dealer.hand.addCard(new Card('diamonds', '2'));
      this.dealer.hand.addCard(new Card('clubs', '10'));
      this.dealer.hand.addCard(new Card('diamonds', '5'));
      this.turn = 'dealer';
      this.status = GameStatus.Started;
      this.score = { player: 12, dealer: 17 };
    });
  
    jest.spyOn(Game.prototype, 'resetGame').mockRestore();
    jest.spyOn(gameService, 'getGameState').mockReturnValue(Promise.resolve(gameService.getGameState()));
  });
